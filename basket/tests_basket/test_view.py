...
#         bucket = product_models.Bucket.objects.create(session_key=self.request.session.session_key)
#
#         req = self.request.post(reverse('to_bucket'), data={'product': self.product.slug,
#                                                             'count': 3},
#                                 HTTP_X_REQUESTED_WITH='XMLHttpRequest')
#         views.add_to_bucket(req)
